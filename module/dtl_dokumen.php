<?php
    ...
    if(isset($_GET['id']) && isset($_GET['folder'])){
        ...
        $queryDtl = mysqli_query($conn, "SELECT folder.*, id_dokumen, kd_folder, nm_dokumen, thn_dokumen,
                                                upload.nama as yg_upload, waktu_diupload,
                                                rubah.nama as yg_rubah, waktu_dirubah, expired, file,
												datediff(expired, now()) as selisih
                                            FROM dokumen
        ...
        $hari_expired = date("l", strtotime($dataDtl['expired']));
            if ($hari_expired == "Sunday") $hari_expired = "Minggu";
            else if ($hari_expired == "Monday") $hari_expired = "Senin";
            else if ($hari_expired == "Tuesday") $hari_expired = "Selasa";
            else if ($hari_expired == "Wednesday") $hari_expired = "Rabu";
            else if ($hari_expired == "Thursday") $hari_expired = "Kamis";
            else if ($hari_expired == "Friday") $hari_expired = "Jumat";
            else if ($hari_expired == "Saturday") $hari_expired = "Sabtu";

        $expired = new DateTime($dataDtl['expired']);
        $sekarang = new DateTime();
        $selisih = $sekarang->diff($expired);
    }
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-hover">
                                        ...
                                        <tr>
                                            ...
                                            <?php if($dataDtl['expired'] == NULL || $dataDtl['expired'] == "0000-00-00"){ ?>
                                                <td>-</td>
                                            <?php }else{ ?>
                                                <td><?= $hari_expired.', '.date('d', strtotime($dataDtl['expired'])).' '.($bln_tgl[date('m', strtotime($dataDtl['expired']))]).' '.date('Y', strtotime($dataDtl['expired'])); ?>&nbsp;&nbsp;(Selisih <?= $selisih->y . " Tahun " . $selisih->m ." Bulan " . $selisih->d . " Hari" ?>)</td>
                                            <?php } ?>
                                        </tr>
                                        <tr>
